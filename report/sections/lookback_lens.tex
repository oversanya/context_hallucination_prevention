...
Context is truncated to 384 tokens and response to 128 tokens independently,
guaranteeing that response token positions are always present in the attention map.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    ROC-AUC                      & 0.625 & 0.706 \\
    F1 at optimal $\tau$         & 0.704 & 0.699 \\
    Precision at optimal $\tau$  & 0.559 & 0.670 \\
    Recall at optimal $\tau$     & 0.950 & 0.730 \\
    Optimal $\tau$               & 0.320 & 0.917 \\
    Feature extraction time      & ${\approx}0.30$\,s/sample & ${\approx}6.1$\,s/sample \\
    \bottomrule
  \end{tabular}
\end{table}

The ROC-AUC of 0.625 is meaningfully above chance (0.5) and approaches the
FActScore-Turbo baseline of 0.706, despite \texttt{opt-125m} not being the model
that generated the RAGTruth responses.  The improvement from 0.555 (initial run,
contaminated by 35.5\% zero-feature vectors due to a context-truncation bug) to 0.625
confirms that the original result was dominated by the implementation defect rather than
fundamental signal absence.  The F1 of 0.704 at the optimal threshold $\tau = 0.320$
is comparable to FActScore-Turbo's 0.699, though with lower precision (0.559 vs 0.670)
and higher recall (0.950 vs 0.730).
